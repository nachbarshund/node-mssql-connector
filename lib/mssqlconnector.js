(function() {
  var ConnectionPool, DataTypes, MSSQLConnector, MSSQLRequestBase, MSSQLRequestStoredProd, Request, connectionpool, extend, _,
  ...
  module.exports = MSSQLConnector = (function(_super) {
    ...
    */
    ...
    MSSQLConnector.prototype._initConnection = function() {
      var _this = this;
      this.isinit = true;
      connectionpool = new ConnectionPool(this.config.poolconfig, this.config.connection);
      connectionpool.connectiontries = 0;
      connectionpool.on("error", function(err, connection) {
        if (err.name.toLowerCase() === "connectionerror") {
          connectionpool.connectiontries++;
          if (_this.config.poolconfig.tries === connectionpool.connectiontries) {
